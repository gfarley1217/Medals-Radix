...
function App() {
  ...
  const apiEndpoint = "https://medalsapifinal-bme8brfwg5dedvan.westus3-01.azurewebsites.net/Api/country";
  const hubEndpoint = "https://medalsapifinal-bme8brfwg5dedvan.westus3-01.azurewebsites.net/medalsHub";
  const userEndpoint = "https://medalsapifinal-bme8brfwg5dedvan.westus3-01.azurewebsites.net/api/user/login";
  ...
  function toUiCountry(apiCountry) {
    const uiCountry = {
      id: apiCountry.id,
    ...
    medals.current.forEach((medal) => {
      const count = apiCountry[medal.name] ?? 0;
      uiCountry[medal.name] = {
        page_value: count,
        saved_value: count,
      };
    });
  ...
  useEffect(() => {
    async function fetchCountries() {
      ...
      const uiCountries = fetchedCountries.map(toUiCountry);
      ...
      connection
        ...
        .then(() => {
          ...
          connection.on("ReceiveAddMessage", (country) => {
            const newCountry = toUiCountry(country);
          ...
          connection.on("ReceivePatchMessage", (country) => {
            const updatedCountry = toUiCountry(country);
            let mutableCountries = [...latestCountries.current];
